...
Solution TrieformProverK::prove(int depth, literal_set assumptions) {
    ...
    if (!solution.satisfiable) { // line 3
        // prover->reduce_conflict(solution.conflict);
        updateSolutionMemo(assumptions, assumptionsBitset, solution);
        return solution; // line 4
    }
    //everything outside of the if statement is part of the "else"
    ...
    triggeredBoxes = prover->getTriggeredBoxClauses(); //create B

    //required to handle multimodal logics
    //can be used for monomodal if there is only one modality to consider
    for (auto modalitySubtrie : subtrieMap) {
        std::cout << modalitySubtrie.first << "\n";
